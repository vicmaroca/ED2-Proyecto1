/*
 * File:   master_plus_ultra.c
 * Author: pablo
 *
 * Created on August 22, 2023, 9:29 PM
 */
...
void main(void) {
    ...
    while(1){
            ...
            char esclavo[] = {c_ultra_slave+48, d_ultra_slave+48, u_ultra_slave+48,'|', '\0'};
            //IMPLEMENTANDO LÓGICA DE FRENADO
            if (ultra_slave >= 2 && ultra_slave <= 15){
                PORTA = 0b11000011;         //Se enciende luces de frenado
                writeSlave(0x50, 170);      //Enviando dato para que apague motor
